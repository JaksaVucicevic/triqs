 ...
 *-----------------------------------------------------------------------------------------------------*/
 ...
 gf_view<M, matrix_valued, S, E> slice_target(gf_view<M, T, S, E> g, Args &&... args) {
  static_assert(std::is_same<T, matrix_valued>::value, "slice_target only for matrix_valued GF's");
  using arrays::range;
  return {g.mesh(), g.data()(range(), std::forward<Args>(args)...), slice_target(g.singularity(), std::forward<Args>(args)...),
          g.symmetry(), slice(g.indices(), std::forward<Args>(args)...), g.name};
 }

 template <typename M, typename T, typename S, typename E, typename... Args>
 gf_const_view<M, matrix_valued, S, E> slice_target(gf_const_view<M, T, S, E> g, Args &&... args) {
  static_assert(std::is_same<T, matrix_valued>::value, "slice_target only for matrix_valued GF's");
  using arrays::range;
  return {g.mesh(), g.data()(range(), std::forward<Args>(args)...), slice_target(g.singularity(), std::forward<Args>(args)...),
          g.symmetry(), slice(g.indices(), std::forward<Args>(args)...), g.name};
 ...
 template <typename M, typename T, typename S, typename E, typename... Args>
 gf_view<M, matrix_valued, S, E> slice_target(gf<M, T, S, E> &g, Args &&... args) {
  return slice_target(g(), std::forward<Args>(args)...);
 }

 template <typename M, typename T, typename S, typename E, typename... Args>
 gf_const_view<M, matrix_valued, S, E> slice_target(gf<M, T, S, E> const &g, Args &&... args) {
  return slice_target(g(), std::forward<Args>(args)...);
 }
 ...
 gf_view<M, scalar_valued, S> slice_target_to_scalar(gf_view<M, Target, S, E> g, Args &&... args) {
 ...
 template <typename M, typename Target, typename S, typename E, typename... Args>
 gf_const_view<M, scalar_valued, S> slice_target_to_scalar(gf_const_view<M, Target, S, E> g, Args &&... args) {
  static_assert(std::is_same<Target, matrix_valued>::value, "slice_target only for matrix_valued GF's");
  using arrays::range;
  return {g.mesh(),
          g.data()(range(), std::forward<Args>(args)...),
 ...
 template <typename M, typename Target, typename S, typename E, typename... Args>
 gf_view<M, scalar_valued, S> slice_target_to_scalar(gf<M, Target, S, E> &g, Args &&... args) {
  return slice_target_to_scalar(g(), std::forward<Args>(args)...);
 }

 template <typename M, typename Target, typename S, typename E, typename... Args>
 gf_const_view<M, scalar_valued, S> slice_target_to_scalar(gf<M, Target, S, E> const &g, Args &&... args) {
  return slice_target_to_scalar(g(), std::forward<Args>(args)...);
 }
 ...
 template <typename M, typename S, typename E>
 gf_view<M, matrix_valued, S, void> reinterpret_scalar_valued_gf_as_matrix_valued(gf<M, scalar_valued, S, E> &g) {
  return reinterpret_scalar_valued_gf_as_matrix_valued(g());
 }

 template <typename M, typename S, typename E>
 gf_const_view<M, matrix_valued, S, void> reinterpret_scalar_valued_gf_as_matrix_valued(gf<M, scalar_valued, S, E> const &g) {
  return reinterpret_scalar_valued_gf_as_matrix_valued(g());
 }
